"""
Texture baking module for Meshine Shop (Phase 2c + 2d).
...
Generates five PBR texture maps from the UV-mapped decimated mesh:
    ...
    roughness.png — Surface roughness estimated from the baked albedo using
                    HSV analysis. Bright + desaturated pixels (polished surfaces)
                    get low roughness; dark or saturated pixels (matte/organic)
                    get high roughness. AO crevices get a slight roughness boost.
                    Resolution: 2048×2048 greyscale PNG.

    metallic.png  — Metallic/non-metallic map estimated from the baked albedo.
                    Metals appear bright and desaturated (chrome, silver, steel).
                    A conservative threshold prevents false positives on white
                    painted or overexposed non-metallic surfaces.
                    Resolution: 2048×2048 greyscale PNG.

albedo/normal/ao are generated by UV-space triangle rasterization: for each
triangle in UV space, pixels are filled by scan-conversion and per-vertex
data is interpolated via barycentric coordinates.

roughness/metallic are generated in image space directly from albedo.png —
no UV rasterization pass needed since the color data is already in 2D texture
space at full 2048×2048 resolution.
...
Public API (called by engine bake_textures() implementations):
    ...
    bake_roughness_map(albedo_path, ao_path, output_path, image_size) → PIL Image
...
# ---------------------------------------------------------------------------
# Roughness map estimation (Phase 2d)
# ---------------------------------------------------------------------------
...
def bake_roughness_map(albedo_path, ao_path, output_path,
    ...
    2. Specularity proxy = Value × (1 − Saturation).
       - Bright + desaturated pixel → high specularity → low roughness
         (polished plastic, metal, glass).
       - Dark or saturated pixel → low specularity → high roughness
         (matte paint, organic surfaces, fabric).
    3. roughness = 1.0 − specularity.
    4. Blend with AO: crevices (low AO) are physically rougher, so they
       receive a +15% roughness boost at maximum occlusion.
    ...
    Returns:
        PIL Image (greyscale 'L', image_size×image_size).
    """
    # --- Load albedo as float32 RGB array ---
    albedo_img = Image.open(albedo_path).convert("RGB")

    # Resize to the target image_size in case albedo was baked at a different
    # resolution. LANCZOS (Sinc) gives the cleanest downscale quality.
    if albedo_img.size != (image_size, image_size):
        albedo_img = albedo_img.resize((image_size, image_size), Image.LANCZOS)

    albedo = np.array(albedo_img, dtype=np.float32) / 255.0  # (H, W, 3) in [0, 1]
    r, g, b = albedo[..., 0], albedo[..., 1], albedo[..., 2]

    # --- Compute HSV components (vectorised, no extra dependencies) ---
    # Value: the brightest channel in the pixel.
    max_c = np.maximum(np.maximum(r, g), b)   # (H, W) — Value (V)
    min_c = np.minimum(np.minimum(r, g), b)   # (H, W)
    delta = max_c - min_c                     # (H, W) — chroma range

    # Saturation: ratio of color range to max brightness.
    # Fully grey (max_c == 0) → saturation = 0.
    saturation = np.where(max_c > 1e-6, delta / max_c, 0.0).astype(np.float32)
    value = max_c  # (H, W) — brightness [0, 1]

    # --- Specularity proxy and roughness ---
    # A bright, desaturated pixel resembles a polished/specular surface.
    # A dark or saturated pixel resembles a matte/diffuse surface.
    specularity = value * (1.0 - saturation)   # (H, W) in [0, 1]
    roughness = 1.0 - specularity              # (H, W) in [0, 1]

    # --- AO blend: crevices are slightly rougher than exposed surfaces ---
    # Load and resize AO map if available. Invert so that dark AO (occluded)
    # contributes a positive roughness boost (max 0.15 at full occlusion).
    if ao_path is not None and Path(ao_path).exists():
        ...
        ao = np.array(ao_img, dtype=np.float32) / 255.0  # (H, W) in [0, 1]
        roughness = roughness + (1.0 - ao) * 0.15        # crevice boost

    # Clamp to physically plausible range.
    # Lower bound 0.2: nothing in a photogrammetry scan is a perfect mirror.
    # Upper bound 1.0: fully matte.
    roughness = np.clip(roughness, 0.2, 1.0)

    # Convert to 8-bit greyscale and save.
...
# ---------------------------------------------------------------------------
# Metallic map estimation (Phase 2d)
# ---------------------------------------------------------------------------
...
def bake_metallic_map(albedo_path, output_path,
    ...
    Algorithm:
    1. Convert albedo RGB to HSV (same vectorised approach as roughness).
    2. Metallic score = (1 − Saturation) × Value.
       - Metals (chrome, silver, steel) appear bright and desaturated:
         high Value, near-zero Saturation → score close to 1.0.
       - Non-metals (paint, plastic, organic) are either saturated (colored)
         or dark → score well below the threshold.
    3. Soft threshold centred at 0.7 with width 0.2:
       - score ≤ 0.7  → metallic = 0.0 (non-metallic)
       - score ≥ 0.9  → metallic = 1.0 (fully metallic)
       - 0.7 < score < 0.9 → smooth blend between the two
    4. Conservative threshold prevents false positives on white painted or
       overexposed non-metallic surfaces (which can appear desaturated).
    ...
    # --- Compute HSV (same approach as roughness map) ---
    ...
    # --- Metallic score and soft threshold ---
    # Bright + desaturated = potentially metallic.
    metallic_score = (1.0 - saturation) * value  # (H, W) in [0, 1]

    # Soft threshold: linear ramp from 0 at score=0.7 to 1 at score=0.9.
    # Conservative — avoids marking white/grey non-metallic surfaces as metal.
