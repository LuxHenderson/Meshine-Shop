...
### Phase 2k Complete — Selective PBR Correction (Material-Aware Chrome Fix)
- **Root cause identified from user screenshot:** After the Voronoi mosaic was eliminated, the model appeared as liquid chrome — correct geometry, correct UV mapping, but the entire surface reflecting light like polished metal regardless of material type
- **Why it happened:** Apple Object Capture's `_roughness0.png` and `_metalness0.png` are calibrated for RealityKit's internal renderer. When fed directly into a standard glTF/game-engine PBR system (three.js, Unreal Engine, Blender Eevee), roughness values near 0 produce mirror-like reflections and elevated metallic values replace diffuse albedo colour with silver specular shading
- **First-pass solution revealed a new problem:** A uniform correction (roughness floor + metallic scale-down applied to all pixels equally) fixed the leather and skin but destroyed the genuinely metallic surfaces. Scissors and metal buckles — which Apple correctly identifies as metallic (0.5–1.0) — were scaled to 2% metallic, making chrome scissors look identical to dark leather. Confirmed by inspecting the embedded GLB binary: metallic max was 0.020 across the entire mesh
- **Final solution — metallic-mask-guided selective correction:** Load the metallic map and build a boolean mask (`metallic > 0.30`). Apply roughness floor (0.60) and metallic clamp (0.0) only to organic zones (leather, skin, fabric). Leave metal zones untouched — scissors stay chrome, buckles stay shiny. The threshold of 0.30 cleanly separates Apple's organic estimates (~0.0–0.15) from its genuine metal estimates (~0.50–1.0)
- **Design choice:** Applied after the texelspace bake as a final post-process, engine-specific to the Apple path. Raw USDZ values are not permanently altered

### Phase 2l Complete — Albedo Clarity Enhancement (Dark Subject Visibility)
- **Root cause:** Photogrammetry captures subject albedo under real-world directional lighting. For dark subjects (dark leather costume, black hair), the captured sRGB values cluster around 0.08–0.15. When a PBR renderer decodes these to linear for diffuse shading, they become 0.004–0.018 (near-black). At that linear albedo, even a single directional light's specular contribution completely overwhelms the diffuse signal — the material appears chrome/wet regardless of roughness or metallic values
